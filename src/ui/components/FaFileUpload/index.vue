<i18n lang="json">
{
  "zh-cn": {
    ...
    "extTip": "支持 {ext} 格式",
    "sizeTip": "大小不超过 {size}",
    "countTip": "数量不超过 {max} 个",
    "typeError": "上传文件只支持 {ext} 格式",
    "sizeError": "上传文件大小不能超过 {size}",
  ...
  "zh-tw": {
    ...
    "extTip": "支持 {ext} 格式",
    "sizeTip": "大小不超過 {size}",
    "countTip": "數量不超過 {max} 個",
    "typeError": "上傳文件只支持 {ext} 格式",
    "sizeError": "上傳文件大小不能超過 {size}",
  ...
  "en": {
    ...
    "extTip": "Support {ext} format",
    "sizeTip": "Size not exceeding {size}",
    "countTip": "Number not exceeding {max}",
    "typeError": "Upload files only support {ext} format",
    "sizeError": "Upload file size cannot exceed {size}",
  ...
  for (const file of selectedFiles.slice(0, remain)) {
    ...
    if (props.size > 0) {
      if (file.size > props.size) {
        toast.error(t('sizeError', {size: filesize(props.size, {standard: 'jedec'})}))
        continue
      }
    }
...
function uploadFile(file: File, index?: number) {
  ...
  axios({
    url: (import.meta.env.DEV && import.meta.env.VITE_OPEN_PROXY) ? '/proxy' + props.action : import.meta.env.VITE_APP_API_BASEURL + props.action,
    method: props.method,
    headers: headersObj,
    data: formData,
    ...
    .then(async (response) => {
      ...
      fileList.value[currentFileIndex].id = response.data.data.id
  ...
  <div class="space-y-2">
    ...
    >
      ...
      <div class="text-sm text-card-foreground/70">
        {{ t('uploadDragTips') }}<span class="cursor-pointer text-primary font-bold">{{ t('uploadClickTips') }}</span>
      </div>
      <input ref="fileInputRef" type="file" :multiple="props.multiple" :disabled="props.disabled" class="hidden"
             @change="e => onSelectFile((e.target as HTMLInputElement).files)">
    </button>
    <div v-if="!props.hideTips && !props.disabled"
         class="flex flex-wrap gap-1 text-xs text-card-foreground/50 empty:hidden">
      <div v-if="props.ext.length > 0" class="after:content-[';_'] last:after:content-empty">
        {{ t('extTip', {ext: props.ext.join(' / ')}) }}
      </div>
      <div v-if="props.size > 0" class="after:content-[';_'] last:after:content-empty">
        {{ t('sizeTip', {size: filesize(props.size, {standard: 'jedec'})}) }}
      </div>
      <div v-if="props.max > 0" class="after:content-[';_'] last:after:content-empty">
        {{ t('countTip', {max: props.max}) }}
      </div>
    </div>
    <div v-if="fileList.length > 0" class="grid grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-2">
      ...
      >
        ...
        <div v-if="item.status === 'uploading'" class="pointer-events-none absolute inset-0 z-0 bg-primary/5"
        ...
        <FaButtonGroup v-if="!props.disabled"
                       class="absolute inset-e-2 top-1/2 opacity-0 transition-opacity -translate-y-1/2 group-hover/file-upload-item:opacity-100">
          <FaButton v-if="item.status === 'error'" variant="outline" size="icon"
                    @click.stop="uploadFile(item.file!, index)">
