...
const modelValue = defineModel<boolean>('modelValue', { default: true })
...
<template>
  <Transition name="loading" appear @after-leave="emits('afterLeave')">
    <div v-if="modelValue" class="fixed bottom-0 top-0 z-10010 w-full bg-background/50 backdrop-blur-sm">
      <div class="absolute left-1/2 top-1/2 flex-col-center gap-4 -translate-x-1/2 -translate-y-1/2">
        ...
        <span v-if="text" class="text-sm text-primary font-bold">{{ text }}</span>
      </div>
    </div>
  </Transition>
</template>
