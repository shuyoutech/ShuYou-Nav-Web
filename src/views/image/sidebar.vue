...
const menuItems = [
  ...
  },
  {
    label: '图生图',
    value: 'ImageToImage',
    path: '/image/image-to-image',
    color: 'linear-gradient(135deg, #f093fb 0%, #f5576c 100%)',
  },
  {
    label: '图片理解',
    value: 'ImageUnderstanding',
...
/* 图生图图标 - 图片层叠 */
.imagetoimage.icon-shape::before {
  content: '';
  width: 16px;
  height: 12px;
...
/* 图片理解图标 - 智能眼镜 */
.imageunderstanding.icon-shape::before {
  content: '';
  width: 18px;
  height: 10px;
  border: 2px solid #667eea;
...
.imageunderstanding.icon-shape::after {
  content: '';
  position: absolute;
  width: 3px;
  height: 3px;
...
/* 图标动画效果 */
.menu-item:hover .texttoimage .icon-shape::before {
  animation: paintSwipe 0.6s ease-in-out;
}

.menu-item:hover .imagetoimage .icon-shape::after {
  animation: slideRight 0.6s ease-in-out;
}

.menu-item:hover .imageunderstanding .icon-shape::after {
