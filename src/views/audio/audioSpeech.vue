...
import type {ModelParameters, ModelVo} from "@/api/ai/model/types.ts";
...
// 音色列表
const voiceOptions = ref<Options[]>([])
// 语言列表
const languageOptions = ref<Options[]>([])
const loadAudioModelList = () => {
  queryHotDataApi('ai_model_audioSpeech').then((res: any) => {
...
const generateSpeech = async () => {
  ...
  try {
    let audioParam: AudioModelParam = {
      ...
      languageType: formData.language,
    }
    ...
    faToast.error('合成失败，请检查网络连接或稍后重试')
  }
}
...
const downloadAudio = () => {
  if (generatedAudio.value) {
    // 在新标签页中打开音频链接
    window.open(generatedAudio.value, '_blank')
  }
}
    ...
    try {
      if (navigator.share) {
        await navigator.share({
          title: 'AI合成的语音',
          text: '听听我用AI合成的语音！',
          url: generatedAudio.value
        })
      } else {
        // 复制链接到剪贴板
        await navigator.clipboard.writeText(generatedAudio.value)
        faToast.success('音频链接已复制到剪贴板')
      }
    } catch (error) {
...
const selectModel = (model: ModelVo) => {
  ...
  formData.language = ''
  if (modelParams.value.voices) {
    for (let i = 0; i < modelParams.value.voices.length; i++) {
      let voices = modelParams.value.voices[i].split('-')
      if (voices.length >= 2) {
        voiceOptions.value.push({label: voices[1], value: voices[0], sort: i})
      }
    }
    // 设置默认选中第一个音色
    if (voiceOptions.value.length > 0) {
      formData.voice = voiceOptions.value[0].value
    }
  }
  if (modelParams.value.languageTypes) {
    for (let i = 0; i < modelParams.value.languageTypes.length; i++) {
      let languages = modelParams.value.languageTypes[i].split('-')
      if (languages.length >= 2) {
        languageOptions.value.push({label: languages[1], value: languages[0], sort: i})
      }
    }
    if (languageOptions.value.length > 0) {
      formData.language = languageOptions.value[0].value
    }
  }
  ...
  <div class="text-to-image-container">
    ...
    <div class="input-panel">
      <div class="panel-body">
        ...
        <div class="form-section">
          ...
          <select v-model="formData.voice" class="form-select">
        ...
        <!-- 语言选择 -->
        <div v-show="languageOptions.length > 0" class="form-section">
          <label class="form-label">
            语言选择
          </label>
    ...
    <div class="result-panel">
      <div class="display-header">
        <h3 class="display-title">合成结果</h3>
      </div>
      ...
      <div class="display-content">
        <div v-if="isLoading" class="loading-state">
          <div class="loading-animation">
            ...
            <p class="loading-text">AI正在合成语音中，请稍候...</p>
          </div>
        </div>

        <div v-else-if="generatedAudio" class="audio-result">
          <div class="audio-container">
            ...
            <div class="audio-actions">
              ...
              <button @click="shareAudio" class="share-button">
        ...
        <div v-else class="empty-state">
          ...
          <p class="empty-text">输入文本内容，开始合成您的专属语音</p>
        </div>
      </div>
    </div>
  </div>
...
.audio-actions .download-button,
.audio-actions .share-button {
  display: flex;
  align-items: center;
  gap: 10px;
  padding: 16px 32px;
...
.audio-actions .download-button:hover {
  transform: translateY(-3px) scale(1.02);
  box-shadow: 0 12px 35px rgba(139, 92, 246, 0.5);
  background: linear-gradient(135deg, #7c3aed 0%, #6d28d9 100%);
}
...
.audio-actions .share-button:hover {
  background: #8b5cf6;
  color: white;
  transform: translateY(-3px) scale(1.02);
  box-shadow: 0 8px 25px rgba(139, 92, 246, 0.4);
