...
// 移除v4版本历史数据
if (!localStorage.has('v5')) {
  localStorage.removeItem('preferences')
  localStorage.setItem('v5', '')
  location.reload()
}
...
function randomTheme() {
  ...
  appSettingsStore.updateSettings({
    theme: {
      ...(colorScheme === 'light' && { light: themeList[Math.floor(Math.random() * themeList.length)] as keyof typeof themes }),
      ...(colorScheme === 'dark' && { dark: themeList[Math.floor(Math.random() * themeList.length)] as keyof typeof themes }),
      colorScheme,
      radius: ([0, 0.25, 0.5, 0.75, 1] as const)[Math.floor(Math.random() * 5)],
    },
    menu: {
      mode: (['side', 'head', 'single', 'only-side', 'only-head', 'side-panel', 'head-panel'] as const)[Math.floor(Math.random() * 7)],
      style: (['', 'arrow', 'line', 'dot'] as const)[Math.floor(Math.random() * 4)],
      subMenuCollapse: Math.random() > 0.5,
      subMenuAutoCollapse: Math.random() > 0.5,
    },
    topbar: {
      switchTabbarAndToolbar: Math.random() > 0.5,
    },
    toolbar: {
      breadcrumb: {
        style: (['classic', 'modern'] as const)[Math.floor(Math.random() * 2)],
      },
    },
  })
}
